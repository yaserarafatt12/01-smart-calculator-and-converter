...
import { Smartphone, Download, X, Share, PlusSquare, ArrowDown } from 'lucide-react';
...
export const PwaInstallPrompt: React.FC<PwaInstallPromptProps> = ({ language = 'id' }) => {
  ...
  useEffect(() => {
    ...
    // Show prompt banner automatically after 4 seconds
    ...
    <>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] text-slate-300 font-medium mt-0.5">
                {isIOS
                  ? (language === 'id' ? 'Tambahkan ke Layar Utama iPhone' : 'Add to iPhone Home Screen')
      ...
      {showIOSModal && (
        <div
          ...
          className="fixed inset-0 z-[110] bg-black/70 backdrop-blur-md flex flex-col justify-end p-4 animate-in fade-in duration-200 cursor-pointer font-sans"
        >
          <div
          ...
          >
            <div className="flex items-center justify-between pb-2 border-b border-slate-800">
              <span className="text-xs font-extrabold text-indigo-400 uppercase tracking-wider flex items-center gap-1.5">
                ...
                {language === 'id' ? 'Cara Pasang di iPhone' : 'How to Install on iPhone'}
              </span>
              <button
                type="button"
                onClick={() => setShowIOSModal(false)}
            ...
            <div className="space-y-3 text-xs font-medium text-slate-200">
              <div className="flex items-center gap-3 p-2.5 rounded-2xl bg-slate-800/80 border border-slate-700/60">
                <div className="p-2 rounded-xl bg-indigo-600 text-white font-black shrink-0">1</div>
                <div>
                  Ketuk tombol <strong className="text-indigo-400 font-extrabold inline-flex items-center gap-1">Bagikan / Share <Share className="w-3.5 h-3.5 inline" /></strong> di menu peramban Safari (bawah/atas).
                </div>
              </div>

              <div className="flex items-center gap-3 p-2.5 rounded-2xl bg-slate-800/80 border border-slate-700/60">
                <div className="p-2 rounded-xl bg-indigo-600 text-white font-black shrink-0">2</div>
                <div>
                  Geser ke bawah lalu ketuk <strong className="text-emerald-400 font-extrabold inline-flex items-center gap-1">"Tambah ke Layar Utama" <PlusSquare className="w-3.5 h-3.5 inline" /></strong>.
                </div>
              </div>
            </div>
            ...
            <div className="pt-2 text-center flex flex-col items-center">
              <p className="text-[11px] text-slate-400 mb-1">
                {language === 'id' ? 'Aplikasi akan otomatis muncul di Layar Utama HP Anda!' : 'Icon will appear on your home screen!'}
              </p>
